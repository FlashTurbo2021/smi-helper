...
print(curcolor..'[SMI-HELPER] {FFFFFF}Проверка библиотек...')
for lib, bool in pairs(bLib) do 
	if not bool then
		if not isMonetLoader() then
			error('{FF0000}ERROR\n\n'..curcolor..'[SMI-HELPER] {FFFFFF}Библиотека "' .. lib .. '" не найдена. Скрипт не может быть запущен\n'..curcolor..'[SMI-HELPER] {FFFFFF}Скачайте библиотеки по ссылке: https://github.com/pla1keo/smiplalkeo/raw/main/smiplalkeo_libs.rar\n')
		else
			print('{FF0000}ERROR\n\n'..curcolor..'[SMI-HELPER] {FFFFFF}Библиотека "' .. lib .. '" не найдена. Скрипт не может быть запущен\n'..curcolor..'[SMI-HELPER] {FFFFFF}Скачайте библиотеки по ссылке: https://github.com/pla1keo/smiplalkeo/raw/main/smiplalkeo_libs.rar\n')
		end
		break
...
print(curcolor..'[SMI-HELPER] {FFFFFF}Проверка прошла успешно!')
print(curcolor..'[SMI-HELPER] {FFFFFF}Если у вас произошли ошибки в скрипте то:')
print(curcolor..'[SMI-HELPER] {FFFFFF}Скачайте библиотеки по ссылке: https://github.com/pla1keo/smiplalkeo/raw/main/smiplalkeo_libs.rar')
...
function check_rank(arg)
	if mainIni.config['c_rang_n'] == "nil" then
		sampAddChatMessage("[SMI-HELPER] {FFFFFF}Данный хелпер предназначен для СМИ, учтите это.", curcolor1)
		return true
	elseif mainIni.config['c_rang_n'] < arg then
		sampAddChatMessage("[SMI-HELPER] {FFFFFF}С " .. arg .. " ранга.", curcolor1)
		return false
	else
		return true
	end
				...
				if v[2] == id then
					if tonumber(v[5]) == 0 then
						if arg ~= nil and tonumber(id) < 1000 then
							...
							sampAddChatMessage("[SMI-HELPER] {FFFFFF}Введите: /uninvite [id] [причина]", curcolor1)
						end
					else
						sampAddChatMessage("[SMI-HELPER] {FFFFFF}Данный игрок имеет выговор. Его нельзя уволить командой.", curcolor1)
					end
				end
			end
...
function addWithTag(text)
	sampAddChatMessage("[SMI-HELPER] {FFFFFF}"..text, curcolor1)
end
...
function send_ad()
	if #u8:decode(str(ad_d)) > 6 then
		if #u8:decode(str(ad_d)) <= 80 then
			...
			focus = true
			sampAddChatMessage("[SMI-HELPER] {FFFFFF}Максимальная длина объявления - " .. curcolor.. "80", curcolor1)
		end
	else
		focus = true
		sampAddChatMessage("[SMI-HELPER] {FFFFFF}Минимальная длина объявления - " .. curcolor.. "6", curcolor1)
	end
end
...
local edit_h = imgui.OnFrame(
	...
	function(self)
		...
		imgui.SameLine(60 * MONET_DPI_SCALE)
		if imgui.Button(u8'Млн.$', imgui.ImVec2(50 * MONET_DPI_SCALE, 20 * MONET_DPI_SCALE)) then
		imgui.StrCopy(ad_d, str(ad_d)..u8" Млн.$")
		end		
		...
		if fast_menu == 'sobes' then
			...
			if imgui.Button(faicons("PLAY") .. u8" Начать собеседование", imgui.ImVec2(-1, 25 * MONET_DPI_SCALE)) then
				if mainIni.config.c_nick ~= '' then
					...
					lua_thread.create(function ()
						...
						wait(300)
						sampAddChatMessage("[SMI-HELPER] {FFFFFF}Советуем указать свой русский никнейм. "..curcolor.."/smi {FFFFFF} - "..curcolor.." Профиль", curcolor1)
					end)
				end
			end
			if imgui.Button(faicons('FILE_ALT') .. u8" Попросить документы", imgui.ImVec2(-1, 25 * MONET_DPI_SCALE)) then
				if fastmenuID > -1 and fastmenuID <= 1000 and sampIsPlayerConnected(fastmenuID) then
					...
					sampAddChatMessage("[SMI-HELPER] {FFFFFF}Игрок вышел.", curcolor1)
				end
			end
...
local main_menu = imgui.OnFrame(
	...
	function(self)
		...
		imgui.Begin('SMI-HELPER', window, imgui.WindowFlags.NoResize + imgui.WindowFlags.NoTitleBar)
		addons.AlignedText('SMI-HELPER', 2)
		...
		if smi_menu == 'sobes' then
			...
			if imgui.Button(faicons("QUESTION") .. u8" Попросить документы", imgui.ImVec2(-1, 25 * MONET_DPI_SCALE)) then
				if select_id[0] > -1 and select_id[0] < 1000 and sampIsPlayerConnected(select_id[0]) then
					...
					sampAddChatMessage("[SMI-HELPER] {FFFFFF}Введите id игрока.", curcolor1)
				end
			end
			if imgui.Button(faicons('QUESTION') .. u8" Расскажите о себе", imgui.ImVec2(-1, 25 * MONET_DPI_SCALE)) then
				lua_thread.create(function ()
		...
		if smi_menu == 'math_efir' then 
			...
			if imgui.Button(u8'Начать эфир', (imgui.ImVec2(100 * MONET_DPI_SCALE, 20 * MONET_DPI_SCALE))) then
				if mainIni.config.c_nick ~= nil then
					...
					sampAddChatMessage("[SMI-HELPER] {FFFFFF}У вас не введен ник-нейм, зайдите в "..curcolor.."/smi {FFFFFF}и в настройках измените его!", curcolor1)
				end
			end
			imgui.SameLine()
		...
		if smi_menu == 'about' then
			...
			if not isMonetLoader() then
				...
				if imgui.IsItemClicked(1) then shell32.ShellExecuteA(nil, 'open', '', nil, nil, 1) end
			end
			if not isMonetLoader() then
				if imgui.IsItemHovered() then imgui.SetTooltip(u8"Кликните ЛКМ, чтобы скопировать, или ПКМ, чтобы открыть в браузере")  end
				if imgui.IsItemClicked(0) then setClipboardText("") end
				...
				if imgui.IsItemClicked(0) then openLink("") end
			end
			...
			if imgui.Button(u8'Сообщить о проблеме/баге') then
				if not isMonetLoader() then
					shell32.ShellExecuteA(nil, 'open', '', nil, nil, 1)
				end
			end
		...
		imgui.BeginChild("##фывыф", imgui.ImVec2(-1, -1), true)
			if ae[1] == 1 then
				...
				if imgui.Button(u8'Сохранить') then
					...
					inicfg.save(mainIni, 'smi.ini')
					sampAddChatMessage('[SMI-HELPER] {FFFFFF}Успешно сохранено.', curcolor1)
				end
			end
		imgui.EndChild()
		imgui.End()
...
-- 		sampAddChatMessage("[SMI-HELPER] {FFFFFF}Приветствуем вас: "..curcolor.. u_name .. '[' .. u_id .. ']', curcolor1)
-- 		sampAddChatMessage("[SMI-HELPER] {FFFFFF}Для открытия главного меню введите "..curcolor.."/smi", curcolor1)
-- 	end
-- end
	...
	while true do wait(0)
		...
		if sampIsLocalPlayerSpawned() and spawn then
			...
			if not doesFileExist(getFontsPath()) then
				sampAddChatMessage("[SMI-HELPER] {FFFFFF}Отсутствует необходимый файл для работы скрипта. Загружаю файл..", curcolor1)
				...
				if doesFileExist(getFontsPath()) then
					sampAddChatMessage("[SMI-HELPER] {FFFFFF}Файл скачан успешно!", curcolor1)
				else
...
function expel(params)
	...
	if arg ~= nil and tonumber(id) < 1000 then
		...
		sampAddChatMessage("[SMI-HELPER] {FFFFFF}Введите: /expel [id] [причина]", curcolor1)
	end
end
...
function giverank(params)
	...
	if arg ~= nil and tonumber(id) < 1000 and tonumber(arg) < 10 then
		...
		sampAddChatMessage("[SMI-HELPER] {FFFFFF}Введите: /giverank [id] [ранг]", curcolor1)
	end
end
...
function fwarn(params)
	...
	if arg ~= nil and tonumber(id) < 1000 then
		...
		sampAddChatMessage("[SMI-HELPER] {FFFFFF}Введите: /fwarn [id] [причина]", curcolor1)
	end
end
...
function unfwarn(params)
	...
	if id ~= nil and id ~= '' then
		...
		sampAddChatMessage("[SMI-HELPER] {FFFFFF}Введите: /unfwarn [id]", curcolor1)
	end
end
...
function invite(params)
	...
	if id ~= nil and id ~= '' then
		...
		sampAddChatMessage("[SMI-HELPER] {FFFFFF}Введите: /invite [id]", curcolor1)
	end
end
...
function blacklist(params)
	...
	if id ~= nil and id ~= '' then
		...
		sampAddChatMessage("[SMI-HELPER] {FFFFFF}Введите: /blacklist [id]", curcolor1)
	end
end
...
function unblacklist(params)
	...
	if id ~= nil and id ~= '' then
		...
		sampAddChatMessage("[SMI-HELPER] {FFFFFF}Введите: /unblacklist [id]", curcolor1)
	end
end
...
function funmute(params)
	...
	if id ~= nil and id ~= '' then
		...
		sampAddChatMessage("[SMI-HELPER] {FFFFFF}Введите: /funmute [id]", curcolor1)
	end
end
...
function fmute(params)
	...
	if arg ~= nil and tonumber(id) < 1000 then
		...
		sampAddChatMessage("[SMI-HELPER] {FFFFFF}Введите: /fmute [id] [причина]", curcolor1)
	end
end
